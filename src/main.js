...
$(function() {
  ...
  $('#start_btn').click(function() { 
    if(points.length >= 3) {
      ...
      alert("Commencer par ajouter des villes !");
    }
  });
...
function draw() {
  if(running) {
    GANextGeneration();
    $('#status').text("Il y a " + points.length + " villes sur la carte, "
                      +"Nous sommes a la generation " + currentGeneration + " et a "
                      + mutationTimes + " mutations. Meilleure valeur: "
                      + ~~(bestValue));
  } else {
    $('#status').text("Il y a " + points.length + " villes sur la carte. ")
  }
